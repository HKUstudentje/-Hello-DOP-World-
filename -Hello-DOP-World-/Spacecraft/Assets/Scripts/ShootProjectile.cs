...
public class ShootProjectile : MonoBehaviour {
	...
	void Update ()
    {
            ...
            if (hit.rigidbody != null)
            {
                ...
                if (sc != null)
                {
                    ...
                    if (Time.time > sc.timeTilWeCanShoot)
                    {
                        ...
                        // clearer to make a local variable GameObject instead of long lines
                        // also, you then have a reference to the projectile to grab the components:
                        // GameObject projectile = (GameObject)Instantiate(projectile, projectileSpawner.position, projectileSpawner.rotation)
                        // projectile.GetComponent<Rigidbody>().velocity = transform.forward * psc.projectileSpeed;


                        currentProjectile = projectileList.Count - 1;
                        // why is currentProjectile scoped in whole Instance in stead of in this scope?
                        // also this is an index, and not the actual Projectile
                        // if you get the projectile, the next lines would also be clearer
        ...
        foreach(GameObject projectile in projectileList)
        {
            ...
            // Notes(gb): don't grab the component each time from the gameObject
            // What if the list was actually comprised of these ProjectileScripts...
            {
                Debug.Log("Boom");
